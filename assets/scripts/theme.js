    ...
    ImagesLazyLoad.prototype = $.extend({}, ImagesLazyLoad.prototype, {
        ...
        buildSrcset: function($elem, url, type) {
            ...
            if(type === 'bg') {
                ...
                return width > 0 && url.indexOf('{width}') !== -1 ? url.replace('_{width}x.progressive', '') + ' 1x, ' + url.replace('_{width}x.progressive', 'width') + ' 2x' : $elem.attr('data-srcset');
            }
        }
    });
    ...
    PopupQuickView.prototype = $.extend({}, PopupQuickView.prototype, {
        ...
        getProduct: function ($product, success) {
            ...
            if(handle) {
                this.ajax = $.ajax({
                    type: 'GET',
                    url: 'https://'+window.location.host+'/assets/img/produtos/' + handle + '?variant=' + variant,
                    data: {
                        view: 'quick-view'
                    },
                    dataType: 'html',
        ...
        switch: function($product, data) {
            ...
            if($image.length) {
                ...
                if(image && image.src && image.id !== $image.attr('data-image-id')) {
                    var src = Shopify.resizeImage(image.src);
    ...
    ProductOptions.prototype = $.extend({}, ProductOptions.prototype, {
        ...
        _loadJSON: function ($product, json, callback, animate) {
            ...
            if(json) {
                ...
                var xhr = $.ajax({
                    type: 'GET',
                    url: 'https://'+window.location.host+'/assets/img/produtos/' + handle,
                    data: {
                        view: 'get_json'
                    },
                    cache: false,
