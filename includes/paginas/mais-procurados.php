<?
    while ($dados_procurados = mysqli_fetch_assoc($query_procurados)) {
        $nome_produto = $dados_procurados['produto'];
        $nome_produto = ucwords($nome_produto);
        $preco = $dados_procurados['preco'];
        $por = $dados_procurados['por'];
        $qtd = $dados_procurados['qtd'];
        $cor = $dados_procurados['cor'];
        $tamanho = $dados_procurados['tamanho'];
        $id = $dados_procurados['id'];
        $sku = $dados_procurados['sku'];
        $sistema = $dados_procurados['sistema'];
        $url_amigavel = $dados_procurados['url_amigavel'];
        $forma = $dados_procurados['forma'];
        $img = $dados_procurados['img'];
        $img_secundaria = $dados_procurados['img_secundaria'];
        $pagina_referencia = 'produtos';
        
        $url = $url_loja."/produto/".$url_amigavel;
        
        if ($dados_procurados['img'] == '') {
