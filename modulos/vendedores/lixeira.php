<?php
    ...
    if ($acao=="excluir") { 
	  	...
	  	$update = "UPDATE $pagina_referencia SET data_excluir='".date('Y-m-d')."', hora_excluir='".date('H:i:s')."', status='e' WHERE id='".$id."' "  or die(mysqli_error());
		...
		$endereco_ip = gethostbyaddr($_SERVER['REMOTE_ADDR']);
	  	$update = "UPDATE $pagina_referencia SET data_excluir='".date('Y-m-d')."', hora_excluir='".date('H:i:s')."', status='a' WHERE id='".$id."' "  or die(mysqli_error());
