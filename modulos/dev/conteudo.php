                        ...
                        <div class="tab-pane fade" id="variacoes">
                            ...
                            if (mysqli_num_rows($sql_cor)>0){
                                ...
                                while ($dados_cor = mysqli_fetch_assoc($sql_cor)){
                                    $option_cor .= '<option value="'.$dados_cor['id'].'">'.$dados_cor['cor'].'</option>';
                                }
                            }
...
<? }
  ...
  if ($acao=="gravar_editar") {
    ...
    if (($check_cor!='' || $check_tamanho!='') AND isset($contador_variacao) AND $contador_variacao>0){
        $delete = mysqli_query($conexao, "DELETE FROM estoque WHERE id_produto='$id'");
        for ($i = 0; $i < $contador_variacao; $i++){
            ((!isset($_POST['cor'][$i]))?$_POST['cor'][$i] = '':'');
            ((!isset($_POST['tamanho'][$i]))?$_POST['tamanho'][$i] = '':'');
            $insert = mysqli_query($conexao, "INSERT INTO estoque (id_cor, id_tamanho, id_produto, qtd, operacao, tipo, valor, data_cadastro, hora_cadastro, data_editar, hora_editar, ip, endereco_ip, status) VALUES ('".$_POST['cor'][$i]."', '".$_POST['tamanho'][$i]."', '$id',  '".$_POST['qtd_variacao'][$i]."', '".$_POST['operacao'][$i]."', '".$_POST['tipo'][$i]."', '".$_POST['valor'][$i]."', '".date('Y-m-d')."', '".date('H:i:s')."', '".date('Y-m-d')."', '".date('H:i:s')."', '$ip', '$endereco_ip', 'a')");
        }
    }else{
        $delete = mysqli_query($conexao, "DELETE FROM estoque WHERE id_produto='$id'");
    }
  ...
  if ($acao=="editar") { ?>
    ...
    <div id="wrapper">
        <form method="POST" action="" enctype="multipart/form-data" id="form_produto">
            <div class="row">
              <div class="col-md-12  header-wrapper" style="margin-top:0;">
                  ...
                  <div class="panel-body"> 
                    ...
                    <div class="tab-content">
                        <div class="tab-pane fade active in" id="descritivo">
                            ...
                            <div class="col-md-12">
                                <div class="form-group" id="form_url_amigavel">
                                  <label>URL AMIGÁVEL</label>
                                  <label class="control-label" for="url_amigavel" style="display:none;" id="label_url_amigavel">Url amigável indisponível</label>
                                  <input name="url_amigavel" type="text" class="form-control" id="url_amigavel" placeholder="Ex: nome-do-produto" maxlength="255" value="<?=$url_amigavel;?>" onkeyup="UrlAmigavel(this.value, 'url_amigavel', '1')">
                                </div>
                            </div>
                            <div class="col-md-12"> 
                                <div class="form-group">
                                    ...
                                    <div class="col-lg-12">
                                      <div class="panel panel-default ">
                                        <div class="panel-body">
                                            ...
                                            ?>
                                                <div class="column" draggable="true"><header>Arraste para ordenar</header>
                                                    <div class="col-lg-12 col-md-12 col-sm-12" id="<?=$id_imagem;?>">
                        ...
                        <div class="tab-pane fade" id="variacoes">
                            ...
                            $sql_variacoes = mysqli_query($conexao, "SELECT e.id, e.id_cor, e.id_tamanho, e.qtd, e.operacao, e.tipo, e.valor, e.peso, e.largura, e.altura, e.comprimento, c.cor, t.tamanho FROM cores AS c RIGHT JOIN estoque AS e ON (e.id_cor=c.id) LEFT JOIN tamanhos AS t ON (e.id_tamanho=t.id) WHERE e.id_produto='$id' AND e.status='a' ORDER BY e.id ASC");
                                              ...
                                              ?>
                                                <tr id="variacao_<?=$contador;?>">
                                                    ...
                                                    <td class="align-middle" id="td_cor_<?=$contador;?>" <?echo(($dados['id_cor']==0)?'style="display: none;"':'');?>>
                                                        <?if ($dados['id_cor']>0){?>
                                                          <select class="form-control input-borda" name="cor[]" id="cor_<?=$contador;?>">
                                                              <?=$option_cor;?>
                                                          </select>
                                                          <script>
                                                            $(document).ready(function() {
                                                    ...
                                                    <td class="align-middle" id="td_valor_<?=$contador;?>">
                                                        <div class="form-group input-group mb-0">
                                                            <input type="text" class="form-control input-borda dinheiro" name="valor[]" id="valor_<?=$contador;?>" value="<?=$dados['valor'];?>">
                                                        </div>
                                                    </td>
                                                    <td class="align-middle"><button type="button" class="btn btn-danger" onclick="excluirVariacao(<?=$contador;?>);" id="remover_<?=$contador;?>">Remover</button></td>
                                                </tr>
                                                <?
                                                $contador++;
                                                $contadorA++;
                            ...
                            <input type="hidden" value="<?=(($num_rows>0)?$contador-1:'');?>" id="contador_variacao" name="contador_variacao">
                            <br>
                            <div class="col-md-12" style="margin-top: 15px;">
                                <div class="pull-right">
                                    <a onclick="$('#li_imagens').click();" data-toggle="tab" aria-expanded="false" class="btn btn-default"><i class="fa fa-reply "></i>&nbsp; ANTERIOR</a>
...
function novaVariacao(){
    ...
    if($('#check_cor').is(':checked')){
      cor = '
        <td class="align-middle" id="td_cor_'+contador+'">'+
            '<select class="form-control input-borda" name="cor[]" id="cor_'+contador+'">'+
                '<?=$option_cor;?>'+
            '</select>'+
        '</td>';
    ...
    if($('#check_tamanho').is(':checked')){
      tamanho = '
        <td class="align-middle" id="td_tamanho_'+contador+'">'+
            '<select class="form-control input-borda" name="tamanho[]" id="tamanho_'+contador+'">'+
                '<?=$option_tamanho;?>'+
            '</select>'+
        '</td>';
    ...
    $('#tbody_variacoes').append(
        '<tr id="variacao_'+contador+'">'+
            ...
            '<td class="align-middle" id="td_valor_'+contador+'">'+
                '<div class="form-group input-group mb-0">'+
                    '<input type="text" class="form-control input-borda dinheiro" name="valor[]" id="valor_'+contador+'" value="0,00">'+
                '</div>'+
            '</td>'+
            '<td class="align-middle"><button type="button" class="btn btn-danger" onclick="excluirVariacao('+contador+');" id="remover_'+contador+'">Remover</button></td>'+
        '</tr>');
    ...
    if (this.checked){
        ...
        if (contador>0){
            for (var i = 1; i <= contador; i++){
                $('#td_cor_'+i).html(
                    '<select class="form-control input-borda" name="cor[]" id="cor_'+i+'">'+
                        '<?=$option_cor;?>'+
                    '</select>'
                );
                $('#td_cor_'+i).show();
