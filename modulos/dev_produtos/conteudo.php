    ...
    if (($check_cor!='' || $check_tamanho!='') AND isset($contador_variacao) AND $contador_variacao>0){
        for ($i = 0; $i < $contador_variacao; $i++){
            ...
            if (isset($_POST['estoque'][$i]) AND $_POST['estoque'][$i]!=''){
                $update = mysqli_query($conexao, "UPDATE estoque SET cor='".$_POST['cor'][$i]."', tamanho='".$_POST['tamanho'][$i]."', produto='$id', qtd='".$_POST['qtd_variacao'][$i]."', operacao='".$_POST['operacao'][$i]."', tipo='".$_POST['tipo'][$i]."', valor='".$_POST['valor'][$i]."', data_cadastro='".date('Y-m-d')."', hora_cadastro='".date('H:i:s')."', data_editar='".date('Y-m-d')."', hora_editar='".date('H:i:s')."', ip='$ip', endereco_ip='$endereco_ip', status='a' WHERE id='".$_POST['estoque'][$i]."'");
    ...
    if (!mysqli_query($conexao, $update)) {
      die('Erro: '.mysqli_error($conexao)); 
        ...
        if (isset($_POST['img_destaque']) AND $_POST['img_destaque']!=''){
			...
			$aleatorio = rand(1,999999);
			$nome = "produto-".$id."-".$nome."-".$aleatorio.".jpg";
			
            if (!is_dir($caminho)){ 
                mkdir("$caminho", 0755);
            }
            
            copy($_POST['img_destaque'], $caminho.$nome);
            unlink($_POST['img_destaque']);
            
            mysqli_query($conexao, "UPDATE produto SET img='".$nome."' WHERE id='".$id."' ")  or die(mysqli_error());
		}
		
		if (isset($_POST['img_secundaria']) AND $_POST['img_secundaria']!=''){
		    $caminho = "../assets/img/".$pagina_referencia."/".$id."/";
		    
		    $nome_secundaria = UrlAmigavel($titulo);
			$aleatorio = rand(1,999999);
			$nome_secundaria = "produto-".$id."-".$nome_secundaria."-".$aleatorio.".jpg";
			
            if (!is_dir($caminho)){ 
                mkdir("$caminho", 0755);
            }
            
            copy($_POST['img_secundaria'], $caminho.$nome_secundaria);
            unlink($_POST['img_secundaria']);
            
            mysqli_query($conexao, "UPDATE produto SET img_secundaria='".$nome_secundaria."' WHERE id='".$id."' ")  or die(mysqli_error());
		}
                  ...
                  <div class="panel-body"> 
                    ...
                    <div class="tab-content">
                                                ...
                                                <div class="image_area">
                							        <label for="upload_secundaria" style="width: 100%; display: flex; align-items: center;align-items: flex-start;">
                    							        <div class="col-md-4 padding-img-upload">
                        								    ...
                        								    <img src="<?=$img_secundaria;?>" id="uploaded_secundaria" class="img-responsive" style="border: 1px solid #eee; max-height: 130px;"/>
                        ...
                        <div class="tab-pane fade" id="variacoes">
                            ...
                            $sql_variacoes = mysqli_query($conexao, "SELECT e.id, e.cor AS id_cor, e.tamanho AS id_tamanho, e.qtd, e.operacao, e.tipo, e.valor, e.peso, e.largura, e.altura, e.comprimento, c.cor, t.tamanho FROM cores AS c RIGHT JOIN estoque AS e ON (e.cor=c.id) LEFT JOIN tamanhos AS t ON (e.tamanho=t.id) WHERE e.produto='$id' AND e.status='a' ORDER BY e.id ASC");
